...
def generate_launch_description():
    lds = []  # List of launch actions
    pkg_dir = get_package_share_directory("tb_worlds")

    # Define predefined spawn locations (x, y, theta)
    spawn_locations = {
        "red_block": (0.0, 0.0, 0.0),   # (x, y, yaw)
        "blue_block": (0.0, 0.0, 0.0),# (x, y, yaw)
    }

    # Iterate through the locations and spawn models
    for model_name, (x, y, theta) in spawn_locations.items():
        ...
        lds.append(
            IncludeLaunchDescription(
                ...
                launch_arguments={
                    "world": "",  # Keep empty if using default world
                    "file": model_sdf,
                    "name": model_name,
                    "x": str(x),
                    "y": str(y),
                    "z": "0.06",  # Adjust if model needs to be above ground
                    "Y": str(theta),
                }.items(),
            )
        )
